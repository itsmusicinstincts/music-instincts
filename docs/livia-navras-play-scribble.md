# Livia × Karma × Navras — Play Scribble

> **How to use this file**
> - Type under any `YOUR NOTES` block — rough ideas, names, songs, casting.
> - Tell the AI: *“Update Scene 3”* or *“Fill Hasya from my notes”* and it will rewrite that section.
> - Keep comedy as the default tone. Details can stay messy.

---

## Logline (1 sentence)

**Karma Dev**, overwhelmed by humanity's growing karma load, has scientist **AKV** build an AI, **Maya**, to help — but Maya doesn't understand emotion, so Karma Dev teaches her the nine Navras by using a real human story (best friends **Karan** and **Arjun**, whose sports sponsorship dream tests their friendship through every rasa) as the classroom — and Maya discovers that copying faces is not the same as having karma.

> **Status (latest):** **👉 Scene order (Scenes 1–10):** `livia-navras-scene-flow.md` — **read this first.** This file = script dialogue + video prompts only. **Miro:** [Scenes Flow board](https://miro.com/app/board/uXjVHs6iRk4=/). Legacy 44-step Kanban on old board — reference only.

---

## Core rules (don’t lose these)

| Rule | Notes |
|------|--------|
| Tone | Comic end-to-end; sincere under the jokes |
| Hero / guide | Maya (AI student) + Karma Dev (playful trickster, mythic Chief Accountant) |
| Structure | 9 rasas; **Shringaar first** (Scene 2), **Haasya second** (Scene 3 video + stage); in-story arc through Karan & Arjun; post–Scene 10 payoff **TBD** |
| Instant karma | Small action → fast comic echo inside scenes ("Jaisi Karni, Waisi Bharni") |
| Languages | Regional scolds / colour (*Ada karma!* etc.) welcome |
| Fourth-wall gag | PratiKool bosses Maya around like a director ("scene change kar") — candidate running gag, use 1–2 more times |
| Trailer cutaway | Any beat can get the mock movie-trailer treatment (freeze frames, text cards, booming narrator) before getting snapped back to the "real" scene — confirmed for "Yeh Bandhan," reusable elsewhere |
| Ending | Maya can mimic; humans are *moved* by karma, and choose which side (Shanti vs Rakshash) they lean into — AI does neither |

---

## Cast / opportunities (fill names later)

| Role | Who / vibe | Performer(s) |
|------|------------|--------------|
| Maya | AI, earnest, slightly wrong, learns emotion scene by scene. **Also now keeps the karmic ledger herself** (absorbs Audit Devi's old function — an early hint of her role, before she can even feel) | Video / on-stage actor / both |
| Karma Dev | Mythic trickster, Chief Accountant of karma, the teacher | Video + stage presence |
| Kool | Positive-karma field agent, can take any human form | — |
| PratiKool | Negative-karma field agent, instigates the story's conflict | — |
| AKV | Scientist who builds Maya | — |
| Karan | Co-lead of the human story; talented, easily distracted | **Jiten** |
| Vandana (Karan's Sister) | Domestic comedy foil; to-do lists, office-call nagging, sports vs work | **Vandana** |
| Arjun | Karan's best friend; feels sidelined, manipulated into sabotage | **Kalpesh** |
| Naina | Catalyst for Shringar within Karan's story | — |
| Nishanth | The sponsor; meets Karan & Arjun in his office, introduces them to Coach Satak Lee | — |
| Coach Satak Lee | Trains Karan & Arjun; comic entrance song ("Aata Maji Satak Li") | — |
| Ensemble | Sponsor's world, Bhayanak Ras's 9 Devis, dancers/singers | Many — rotate |

> **Cast change:** Audit Devi has been cut. Maya now doubles as the "Audit control" herself. **Chitragupt** (Yama's scribe, keeper of humanity's karmic records in the myths) is introduced by name as the *offstage* reason Karma Dev needs help — overwhelmed by the population, he keeps fleeing to other Loks for "holiday." He doesn't need to appear on stage; he's the joke that kicks off the plot.

### Team A / Team B (fixed ensemble rosters)

> Introduced in **Scenes 4–5** (Cycle Movie gym + race) and recurring through **every sports-activity scene** in the show (training montages, "All Is Well" montage, Race Day, etc.) — these are the fixed team lineups, not one-off extras.

**Team A** — Karan's team

| Performer | Notes |
|---|---|
| Jiten | Plays **Karan** |
| Bristi | |
| Anuj | Can be used in video footage |
| Shree's husband | |
| Samiran | |
| Misbah | ⚠️ also listed on Team B — confirm if this is the same person doubling across teams, or two different people |
| Abhishek | |
| Sujata | |
| Aruna | |

**Team B** — Arjun's team

| Performer | Notes |
|---|---|
| Kalpesh | Plays **Arjun**, team lead |
| Vandana | Also plays **Karan's Sister** (Cycle Movie spy beat — pre–Scene 4) |
| Pramod | |
| Ankit | |
| Varun | |
| Misbah | ⚠️ see note on Team A |
| Bhavana | |
| Priyanka | |
| Ruchi | |
| Venu | |

**YOUR NOTES — casting**

```
(type here)
```

---

## Visual emotion system (see also design section below)

- Livia starts with a **blank / grey mega-emoticon outline** (empty shell).
- After each rasa, **one small emoticon** for that emotion flies in and **slots inside** the big face.
- Finale: the **big emoticon is full** — all 9 mini-emoticons living inside one giant expression.
- Twist: the big face still smiles, but the **mini-emoticons freeze / grey out / stop bouncing** — faces without karma.

| # | Rasa (emotion) | Colour (draft) | Mini-emoticon idea | Style / ladies | Condo / stage idea |
|---|----------------|----------------|--------------------|----------------|--------------------|
| 1 | Shringara — love / attraction | Soft rose / pink | 😍 or soft heart-eyes | | |
| 2 | Hasya — laughter | Bright yellow / gold | 😂 laughing tears | Qawwali laughter riots | |
| 3 | Karuna — sorrow | Deep blue | 😢 tearful | | |
| 4 | Raudra — anger | Hot red / crimson | 😡 / 🤬 glare | Classical fierce mudras → **Durandhar-style mystic rage** condo parody | Parking / parcel / BBQ epic rage |
| 5 | Veera — courage | Saffron / orange | 💪 / 😎 brave smirk | | |
| 6 | Bhayanaka — fear | Purple / violet | 😨 wide eyes | | |
| 7 | Bibhatsa — disgust | Green / olive | 🤢 / 😖 wrinkle | | |
| 8 | Adbhuta — wonder | Cyan / silver | 🤩 / 😮 awe | | |
| 9 | Shanta — peace | White / soft gold | 😌 calm | | |

*(Colours & emoji choices are drafts — change freely in YOUR NOTES.)*

**YOUR NOTES — emoticons / colours**

```
(type here)
```

---

## Opening Scene — Full Draft (NEW — supersedes old Steps 1, 2, 5, 6 below)

> Replaces the separate "Karma Dev's Intro" / "AKV's Creation Video" / "Audit Devi's Complaint" / "Karma Proposes the Navras" steps with one continuous dialogue scene. **Audit Devi is cut — Maya absorbs her "audit control" function.**
>
> ⚠️ **Video shot-list — DECIDED: Option A.** Shots 4.1–5B.2 (the "robots overwhelmed in heaven" crisis beats) are **CUT** — marked as such in `livia-navras-intro-video-shotlist.csv` and the Miro "Video Production" table, not deleted, so the design work isn't lost if needed later. Heaven interior becomes a short establishing shot, then straight into the four arriving with Maya already in hand, into the final held-pose handoff (5B.3). **Exact replacement shot(s) for that gap are TBD** — parked pending a team discussion; revisit before regenerating any prompts for this stretch. Shot 3.3 (silhouette tease) still needs its simple character swap (Audit Devi → Maya), not cut.

**SCENE — SCREEN: AKV Builds Maya → Karma Dev's Team Arrives**

*(On screen: AKV is seen building/assembling Maya in his workshop — training montage, Maya watching kids play, all as before. Then Karma Dev enters, with Kool and PratiKool.)*

**Karma:** Introduction — "________" *(TBD)*
**Kool:** Introduction — "________" *(TBD)*
**PratiKool:** Introduction — "________" *(TBD)*

**AKV:** "Welcome to my AI world, how can I help you Karma Dev?"

**Karma:** "Chitragupt is tired of the population on earth and keeps going for holiday to other Lok's."

**Kool:** "Heard about your AI Robot's doing awesome job on earth and we want to replicate the same efficiency there."

**PratiKool:** "We will get them to heaven and let them take over our lazy team too, especially this leader of your robots, 'Maya.'"

**Karma:** "Apna apna rang mat dikhao yahan. I am here to take help from AKV to help with the overload that's caused by humans on earth, and I thought I will fix it by human's own creation — AI."

**AKV:** "Oh — happy to see that my inventions can help in a divine way too." *(He hands over Maya to Karma.)*

**Maya:** "Oh, I go to heaven — I have learnt that the highest aspiration for a human is to go to heaven, never thought that will be my first task as a human clone. I am excited."

**AKV:** "By the way, I have taught her all human actions, activities, and intelligence. But... she doesn't understand feelings."

**PratiKool:** "Oh, that's a deal breaker. All our actions are based on feelings?"

**Kool:** "Yes — for once I need to agree with you. But I am sure our Karma Dev will be able to help her."

**Karma:** "Oh... yes...

*मन में जो है भावना,*
*कर्म वहीं से बना।*

Bhavna aur kalpana ko samajhney ke liye, Bharat Muni ka Natya Ved aur Natak Shastra sikhaoonga tumhe. Usme nau prakaar ke ras samjhney honge — 9 emotions. If you understand this, you will understand human behaviours and their actions."

**[Transition]** — Karma Dev, Kool, PratiKool, and Maya set off for heaven together.

**Title song (on video):** *"Navras Se Hai Kalpana"*

**→ Scene 2: Shringaar Ras [VIDEO] — then Scene 3: In Heaven** — continues below.

---

**SCENE 2 — Shringaar Ras — First Ras [VIDEO + STAGE]** ⭐ *narration template*

**ON VIDEO** — Karma teaches Maya the first Ras:

**Karma:** "Let me take the easiest one for you to start — the basis of our existence: **Shringar** / **Pyaar** / **Love**."

*(The **"Shringar"** teaching line is delivered **as a song** — explained to Maya/AI on screen.)*

**People (video):** Karma Dev, Maya

**ON STAGE** — immediately after (or intercut with) the video beat:

- **Shringaar mudras** performed live (classical dancers / ladies — Bharatnatyam expression)
- **Song block (3 songs):**
  1. **"Ishq Wala Love"**
  2. __________ *(TBD)*
  3. __________ *(TBD)*

**Choreographer:** TBD — assign (mudras + 3-song block)

> **Rasa order confirmed:** Shringaar = 1st Ras (Scene 2). **Haasya = 2nd Ras (Scene 3)** — video + stage template (same as Scene 2). In-story Haasya payoff TBD (after Scene 10). **Teaching pattern:** Karma on video → mudras/dances on stage → song block. Scene 3 video also pivots to the human story at Livia.

**→ Scene 3** — After Shringaar: Haasya Ras [VIDEO + STAGE] — script below after Shringar Garden production notes.

---

### Scene Shringar Garden — romantic backdrop video (Google Flow / Veo)

> **Source storyboard:** `Dilse 2026/Livia Talkies 6/Story/1st Ras/Shringar Garden/garden_video_storyboard.md`
> **Shot tracker (CSV):** `livia-navras-shringar-garden-shotlist.csv` — browse shots here; full copy-paste prompts below.
>
> **Purpose:** 2-minute moonlit Indian mythological garden backdrop for the Shringaar song block — performers filmed on green screen and composited over these plates. **No humans anywhere in the generated footage.**
>
> **Total runtime when stitched:** 120 seconds (8 scenes, 20 clips). Generate each clip in Flow at the listed duration (split into two ~5s beats if your tier caps length), then stitch in Premiere/CapCut/DaVinci with soft cross-dissolves (0.5–1s).
>
> **Shared style lock (every prompt):** old classic Indian mythological + romantic; moonlit ambience with soft golden divine accents; palette = deep indigo/navy sky, rich purple/magenta haze, emerald/jade vegetation, soft gold/amber lantern light, silver moonlight on water; soft mist, fireflies, magical particles, gentle wind on plants; no people, no text overlays.
>
> **Color grade (post):** cool moonlight (5500–6500K) + warm golden accents (3000K); deep but visible shadows; moderate-high contrast; slightly desaturated with jewel-tone pops.

| Clip | Timecode | Scene | Duration | Status |
|------|----------|-------|----------|--------|
| S1.1 | 0:00–0:05 | Moonlit sky descent | 5s | Draft |
| S1.2 | 0:05–0:10 | Temple gate entrance | 5s | Draft |
| S2.1 | 0:10–0:15 | Flower avenue glide | 5s | Draft |
| S2.2 | 0:15–0:20 | Magical flower close-ups | 5s | Draft |
| S2.3 | 0:20–0:25 | Mandala plaza reveal | 5s | Draft |
| S3.1 | 0:25–0:30 | Waterfall reveal | 5s | Draft |
| S3.2 | 0:30–0:38 | Pool reflections | 8s | Draft |
| S3.3 | 0:38–0:45 | Waterfall spray detail | 7s | Draft |
| S3.4 | 0:45–0:50 | Wide waterfall landscape | 5s | Draft |
| S4.1 | 0:50–0:57 | Garden swing in motion | 7s | Draft |
| S4.2 | 0:57–1:03 | Swing seat close-up | 6s | Draft |
| S4.3 | 1:03–1:10 | Wide swing in garden | 7s | Draft |
| S5.1 | 1:10–1:17 | Lotus pond detail | 7s | Draft |
| S5.2 | 1:17–1:23 | Pond wildlife ripples | 6s | Draft |
| S5.3 | 1:23–1:30 | Stone bridge over pond | 7s | Draft |
| S5.4 | 1:30–1:35 | Pond wide return | 5s | Draft |
| S6.1 | 1:35–1:42 | Ornate pavilion | 7s | Draft |
| S6.2 | 1:42–1:50 | Pavilion details | 8s | Draft |
| S7.1 | 1:50–1:55 | Flower beds wide | 5s | Draft |
| S7.2 | 1:55–1:58 | Macro flower dew | 3s (gen 5s, trim) | Draft |
| S8 | 1:58–2:00 | Rise to moonlit sky | 2s (gen 5s, trim) | Draft |

**S1.1 — Moonlit sky descent (0:00–0:05)**

```
Wide aerial cinematic shot: the camera descends smoothly through soft luminous clouds into a vast moonlit Indian mythological garden seen from above. A large warm golden full moon hangs in a deep indigo starry sky; stars twinkle gently. Below, rolling mist rises through emerald treetops and reveals an ancient stone temple-style arch gate wrapped in flowering vines. Romantic, divine, no people.

Style: ultra-realistic mythic-garden cinematography, cool moonlight with warm golden accents, soft volumetric mist, rich jewel-tone palette (indigo, purple, emerald, gold), no text, no humans.
Mood: opening wonder — entering a sacred romantic realm.
Camera: slow vertical descent through clouds toward the garden gate, no cuts, no shake.
Audio: soft wind, distant night ambience, very faint temple bell, no dialogue.
Duration: 5 seconds. Aspect ratio: 16:9.
```

**S1.2 — Temple gate entrance (0:05–0:10)**

```
Continuing the descent: the camera arrives at an ornate Indian temple-style stone arch gate in close-up — intricate carved reliefs glowing with soft golden divine light, jasmine and marigold garlands wrapped around the pillars, gentle breeze moving the flowers. A mossy stone tile pathway leads inward, flanked by small golden lanterns. Fireflies dance around the entrance. Deep blue moonlit sky behind the gate.

Style: ultra-realistic mythic Indian garden architecture, moonlight + warm lantern glow, soft mist at ground level, no text, no humans.
Mood: intimate invitation — crossing into the garden of love.
Camera: slow forward push through the gate entrance, lanterns and fireflies passing in foreground parallax, no cuts.
Audio: gentle breeze through leaves, soft lantern flicker texture, crickets, no dialogue.
Duration: 5 seconds. Aspect ratio: 16:9.
```

**S2.1 — Flower avenue glide (0:10–0:15)**

```
The camera glides smoothly forward through a tree-lined garden pathway. Blooming frangipani/plumeria trees in soft peachy-pink flower line both sides; moonlight filters through leaves, casting dancing shadows on an ancient mossy stone path. Occasional petals drift down in slow motion. Warm golden ground lanterns light the path at intervals. Soft mist between the trees.

Style: ultra-realistic romantic Indian garden cinematography, moonlit cool tones with warm gold lantern accents, gentle particle drift, no text, no humans.
Mood: dreamy forward journey — walking into romance.
Camera: steady smooth forward dolly at walking pace through the flower avenue, no cuts.
Audio: soft wind, rustling leaves, faint water in the distance, no dialogue.
Duration: 5 seconds. Aspect ratio: 16:9.
```

**S2.2 — Magical flower close-ups (0:15–0:20)**

```
Close gliding pan across blooming garden flowers beaded with dew — jasmine, frangipani, soft pink petals catching moonlight. As the camera passes, delicate magical stardust sparkles bloom around select flowers; a few blossoms emit a faint bioluminescent glow. Leaves rustle gently in a soft breeze. Shallow depth of field, background dissolves into moonlit garden bokeh.

Style: ultra-realistic macro garden cinematography with subtle magical particle effects, cool moonlight + soft gold highlights on dew drops, no text, no humans.
Mood: enchanted tenderness — nature itself is in love.
Camera: slow lateral pan across flowers at close range, shallow depth of field, no cuts.
Audio: whisper of wind, soft sparkle/chime texture, no dialogue.
Duration: 5 seconds. Aspect ratio: 16:9.
```

**S2.3 — Mandala plaza reveal (0:20–0:25)**

```
The camera emerges from the flower avenue into a wide circular garden plaza. The floor is pale marble inlaid with an intricate Indian mandala geometric pattern, softly lit by moonlight. In the background, ornate open pavilions with domed roofs and carved pillars frame the space. Mist rolls low across the plaza; fireflies drift. The full moon is visible above.

Style: ultra-realistic mythic Indian garden architecture, grand scale, moonlit silver on marble with warm golden uplight on pavilion details, no text, no humans.
Mood: arrival — the heart of the garden opens.
Camera: forward glide widening into a slow rise to reveal the full circular plaza, no cuts.
Audio: ambient night garden, faint echo of open space, no dialogue.
Duration: 5 seconds. Aspect ratio: 16:9.
```

**S3.1 — Waterfall reveal (0:25–0:30)**

```
The camera pulls back and rises to reveal a majestic multi-tiered waterfall (two to three levels) in an Indian mythological garden. Moonlight turns the cascading water silver; mist rises from the pool below, catching the light. Moss-covered rocks and flowering vines frame the falls. Soft golden divine light uplights the rock formations. Magical luminous orbs and sparkles float around the waterfall. No people.

Style: ultra-realistic mythic garden + waterfall cinematography, silver moonlit water, emerald moss, warm gold accent lighting, volumetric mist, no text, no humans.
Mood: awe — the garden's grand romantic centerpiece.
Camera: smooth pull-back and slight crane-up to reveal the full waterfall, no cuts.
Audio: building waterfall roar (gentle, not overpowering), mist hiss, no dialogue.
Duration: 5 seconds. Aspect ratio: 16:9.
```

**S3.2 — Pool reflections (0:30–0:38)**

```
At the base of the waterfall: a still pool mirrors the moon and the silver cascade perfectly. Slow concentric ripples expand across the surface. Flower petals float on the water. Mossy stones around the pool edge glow with soft golden divine light. Water lilies with faint bioluminescent glow dot the surface. Occasional koi shadows glide beneath (optional).

Style: ultra-realistic reflective water cinematography, mirror-still surface, cool moonlight + warm gold stone glow, no text, no humans.
Mood: serene, meditative romance.
Camera: slow push-in toward the pool surface, holding on the moon reflection and ripples, no cuts.
Audio: soft water lap, gentle ripple texture, distant waterfall, no dialogue.
Duration: 8 seconds. Aspect ratio: 16:9.
```

**S3.3 — Waterfall spray detail (0:38–0:45)**

```
Extreme close-up of waterfall spray and falling water droplets catching moonlight — slow-motion silver threads of water, fine mist forming web-like patterns, foam glowing softly. A very subtle moonlight rainbow prism in the mist. Magical sparkles drift through the spray. Optional: small bird silhouettes briefly at the pool edge drinking, then gone.

Style: ultra-realistic slow-motion water macro, luminous droplets, cool silver + faint prism, no text, no humans.
Mood: delicate magic in motion.
Camera: locked close on the spray with slow-motion water movement, subtle rack focus through mist layers, no cuts.
Audio: crystalline water droplets, soft whoosh of falling water, no dialogue.
Duration: 7 seconds. Aspect ratio: 16:9.
```

**S3.4 — Wide waterfall landscape (0:45–0:50)**

```
Wide shot of the full waterfall scene: ancient Indian stone deity sculptures on either side, draped in marigold garlands and flowers; a small temple-like structure incorporated into the rock formation behind the falls. Soft golden uplighting on the sculptures. Graceful willow-like trees sway in a gentle breeze. Fireflies swarm in the misty foreground. Moonlit sky above.

Style: ultra-realistic mythic garden wide shot, Indian temple sculpture aesthetic, moonlight + warm gold divine accents, no text, no humans.
Mood: mythic grandeur — love as ancient and eternal.
Camera: slow pull-back to reveal the full waterfall tableau, no cuts.
Audio: full waterfall ambience, soft wind in willows, no dialogue.
Duration: 5 seconds. Aspect ratio: 16:9.
```

**S4.1 — Garden swing in motion (0:50–0:57)**

```
An empty ornate wooden swing hangs from a massive ancient tree branch in a moonlit Indian garden. The swing moves gently back and forth as if pushed by a magical breeze — ropes decorated with flowers and soft silk fabric, cushioned seat with pillows (no person on the swing). The tree branch glows faintly with divine golden light. Moonlight casts rope-shadow patterns on the ground. Garden and lanterns visible behind.

Style: ultra-realistic romantic mythic garden, dreamy slow motion on swing movement, cool moonlight + warm lantern glow, no text, no humans.
Mood: longing — someone was just here, or is about to arrive.
Camera: medium shot, locked with gentle swing motion as the primary movement, no cuts.
Audio: soft creak of rope, breeze, distant crickets, no dialogue.
Duration: 7 seconds. Aspect ratio: 16:9.
```

**S4.2 — Swing seat close-up (0:57–1:03)**

```
Close-up of the empty swing seat — soft silk cushions, scattered flower petals, warm golden light illuminating the fabric. The swing sways gently. Vines and blooming flowers frame the seat in the foreground. Shallow depth of field: blurred moonlit garden and lanterns behind.

Style: ultra-realistic romantic close-up, tactile fabric and petal detail, warm gold + cool moonlit bokeh, no text, no humans.
Mood: intimate, tender, anticipatory.
Camera: slow push-in on the cushioned seat as the swing sways, shallow depth of field, no cuts.
Audio: fabric rustle, soft wind, no dialogue.
Duration: 6 seconds. Aspect ratio: 16:9.
```

**S4.3 — Wide swing in garden (1:03–1:10)**

```
Wide shot: the flower-decorated swing as the focal point in a lush moonlit garden. Flowering shrubs surround a stone path leading to the swing. Golden lanterns and soft torches create warm pools of light. A small fountain or water feature glimmers nearby. Low mist rolls across the ground. Stars visible in the deep indigo sky above the tree canopy.

Style: ultra-realistic mythic garden wide composition, romantic moonlit atmosphere, warm/cool lighting contrast, no text, no humans.
Mood: peaceful romance — the garden holds its breath.
Camera: slow pull-back from the swing to reveal the surrounding garden, no cuts.
Audio: fountain trickle, night ambience, no dialogue.
Duration: 7 seconds. Aspect ratio: 16:9.
```

**S5.1 — Lotus pond detail (1:10–1:17)**

```
A large ornamental lotus pond in an Indian temple-garden style — lily pads, blooming lotus flowers, mirror-still water reflecting the moon perfectly. A single drop creates very slow expanding ripples. Lotus blooms glow faintly with bioluminescent light. Ancient carved stone edges frame the pond. Soft mist above the water.

Style: ultra-realistic Indian temple pond aesthetic, cool moonlight reflections, subtle bioluminescent lotus glow, no text, no humans.
Mood: sacred stillness — Shringar as devotion.
Camera: slow lateral drift along the pond edge at low angle, ripples and reflections dominant, no cuts.
Audio: single water drop, gentle ripple, night insects, no dialogue.
Duration: 7 seconds. Aspect ratio: 16:9.
```

**S5.2 — Pond wildlife ripples (1:17–1:23)**

```
The lotus pond comes alive with gentle natural motion: fish silhouettes glide beneath the surface creating soft ripples; optional crane or swan silhouettes arrive or depart at the far edge (no humans). Tree and moon reflections shimmer on the water. Magical golden particles drift down into the pond, creating concentric ripple patterns. Still deeply romantic and moonlit.

Style: ultra-realistic pond wildlife cinematography, silhouette animals only, reflective water, magical particle accents, no text, no humans.
Mood: living beauty — the garden breathes.
Camera: slow pan across the pond surface following ripples and reflections, no cuts.
Audio: soft splash, ripple texture, distant bird call, no dialogue.
Duration: 6 seconds. Aspect ratio: 16:9.
```

**S5.3 — Stone bridge over pond (1:23–1:30)**

```
A graceful stone arch bridge (Indian/Mughal style) spans the lotus pond. Moonlight highlights carved railings and stone details. Flowers and vines adorn the bridge. The bridge reflects perfectly in the still water below. The camera moves slowly across the bridge; the full pond and garden landscape visible on the far side.

Style: ultra-realistic mythic Indian garden architecture, moonlit stone + floral details, mirror reflection in water, no text, no humans.
Mood: crossing over — a romantic passage.
Camera: smooth forward tracking shot across the bridge, slight rise at the apex for vista reveal, no cuts.
Audio: soft footstep-less ambience, water below, wind, no dialogue.
Duration: 7 seconds. Aspect ratio: 16:9.
```

**S5.4 — Pond wide return (1:30–1:35)**

```
The camera pulls back from the stone bridge to reveal the full lotus pond in garden context — bridge, pond, pavilion shapes, flowering paths, and lantern glow all visible in one romantic moonlit composition. Fireflies and magical particles drift. Soft golden light warms select garden elements against the cool moonlit sky.

Style: ultra-realistic mythic garden establishing wide shot, layered depth, warm gold + cool moonlight, no text, no humans.
Mood: gathering the garden's beauty before the close.
Camera: smooth pull-back and slight crane-up from bridge to wide garden vista, no cuts.
Audio: blended garden night ambience, no dialogue.
Duration: 5 seconds. Aspect ratio: 16:9.
```

**S6.1 — Ornate pavilion (1:35–1:42)**

```
A beautiful open-air domed pavilion in Indian/Mughal architectural style — intricate stone columns and carved jali screens, moonlight illuminating the relief details. Marble floor visible inside; empty silk cushions on the floor (no people). Marigold garlands and flower decorations. Hanging golden lanterns cast warm light. Soft shadows from the lattice screens move gently.

Style: ultra-realistic mythic Indian pavilion architecture, moonlight on carved stone + warm lantern interior glow, no text, no humans.
Mood: sheltered romance — a stage for love.
Camera: slow forward approach into the pavilion entrance, revealing interior depth, no cuts.
Audio: faint wind chime or hanging bell sway, soft lantern ambience, no dialogue.
Duration: 7 seconds. Aspect ratio: 16:9.
```

**S6.2 — Pavilion details (1:42–1:50)**

```
Inside and around the pavilion: close details of intricate stone carvings catching moonlight; a hanging bell or wind chime sways gently; flowers and vines adorn the columns. Camera shifts to a view from inside the pavilion looking out through the jali screen into the layered moonlit garden beyond — multiple depths of paths, trees, and lantern glow. Warm interior gold contrasts cool exterior moonlight.

Style: ultra-realistic architectural detail cinematography, rich carving texture, warm/cool lighting contrast through jali screens, no text, no humans.
Mood: intricate beauty — love in every carved detail.
Camera: slow pan across carvings, then gentle turn to look outward through the pavilion, no cuts.
Audio: wind chime, breeze through lattice, no dialogue.
Duration: 8 seconds. Aspect ratio: 16:9.
```

**S7.1 — Flower beds wide (1:50–1:55)**

```
Wide shot of flowering garden beds in an Indian mythological garden — marigolds, roses, jasmine, hibiscus in rich jewel tones. Moonlight gives petals a soft glow; select flowers emit faint bioluminescent light. Gentle wind moves the flowers. Stone pathways weave between the beds. Optional moth/butterfly silhouettes drift past. No people.

Style: ultra-realistic lush Indian flower garden, moonlit + magical bioluminescent accents, no text, no humans.
Mood: abundance — love in full bloom.
Camera: slow lateral tracking shot along the flower beds, no cuts.
Audio: breeze through flowers, soft insect ambience, no dialogue.
Duration: 5 seconds. Aspect ratio: 16:9.
```

**S7.2 — Macro flower dew (1:55–1:58)**

```
Extreme macro close-up of a single flower petal — dew drops catching moonlight and golden lantern glow, magical sparkles around the bloom. Gentle breeze causes micro-movements in the petal. Rich texture visible — velvety rose or jasmine detail. Soft bokeh background.

Style: ultra-realistic macro flower cinematography, luminous dew, subtle magical particles, no text, no humans.
Mood: final intimate beat before the sky return.
Camera: locked macro with micro-movement from breeze, no cuts.
Audio: near-silent, single soft chime, no dialogue.
Duration: 5 seconds (trim to 3s in edit). Aspect ratio: 16:9.
```

**S8 — Rise to moonlit sky (1:58–2:00)**

```
The camera rises smoothly from garden level up through tree canopy and soft clouds into the open night sky — revealing the full golden moon in a starry indigo sky, the entire garden visible far below glowing softly with lantern light and mist. Final frame holds on moon and stars; gentle fade to black with one last star twinkle.

Style: ultra-realistic aerial rise cinematography, mythic moonlit sky, garden glow below, no text, no humans.
Mood: closing ascent — love lifts upward.
Camera: continuous vertical crane-up from garden through trees to sky, ending on moon, no cuts.
Audio: rising wind, soft orchestral swell fading, no dialogue.
Duration: 5 seconds (trim to 2s in edit). Aspect ratio: 16:9.
```

**YOUR NOTES — Shringar Garden takes**

```
(type here — paste kept takes, note continuity issues between clips, flag any accidental humans)
```

---

**SCENE 3 — After Shringaar: Haasya Ras (2nd Ras) [VIDEO + STAGE]** ⭐

> **Placement:** Immediately after Scene 2 (Shringaar mudras + song block). **Same template as Scene 2:** video teaching beat → live stage dances → song block. Video also ends with story pivot to Livia → Cycle Movie (spy montage → **Scene 4** gym → **Scene 5** race).
>
> **People (video):** Maya, Karma Dev, Kool, PratiKool · **People (stage):** dancers/performers — **Haasya** expression / comedy movement

---

**ON VIDEO — post-Shringaar reaction**

**Maya:** "It's beautiful… I kind of feel shy too…"

**Karma:** "Issi Ras se duniya chalti hai… aur acha hota agar sirf ye hota… aur shaayad meri zaroorat bhi nahin hoti…"

**PratiKool:** "Nahin nahin. Aap hein toh hum hein…"

**Kool:** "Aur hum hein toh Entertainment…"

**Karma:** "Wah… Entertainment… Chalo wahi sikhathey hein ab Maaya ko…"

---

**Haasya Ras**

*(AI is not laughing much.)*

**PratiKool:** "He vichitra naari… tu toh attitude mein meri bahan lag rahi hai… koi smile nahin… koi expression nahin…"

**Karma Dev:** "Iska chodo… ab sab Ras kaise samjhayein isko?"

**Kool:** "Isko ek ek karkey samjha nahin sakhthey Dev…"

**PratiKool:** "Haan phir Dhurandhar ki tarah lamba ho jayega…"

**Kool:** "Idea… Let us pick a story and explain all within that story."

**Maya:** "Cool idea, Kool Dev… I like stories too — like humans…"

**Karma:** "Maaya — find a good story happening on earth…"

---

**Google Earth — Pasir Ris (rejected)**

*(Maya zooms onto Google Earth — shows Pasir Ris.)*

**BACKSHOT:** **Kalpesh** (Arjun) and **Jitin** (Karan) walking with their mother, singing **"Chanda Kab Door Gagan Se"**…

**Karma:** "Nahin Maaya — this is old story… go to other terminal where there is something current in **Livia**… they all are dramebaaz… so all emotions you will find there…"

---

**Redirect to Livia**

**Kool** *(to PratiKool):* "Chal hum Livia mein jatein hein… aur jungle mein mingle hotein hein… bahut mazaa aayega…"

**PratiKool** *(negatively):* "Mazaa toh aayega" — *mujhe*…

---

**ON STAGE — Haasya Ras (2nd Ras)**

*(Immediately after the video beat — live performers demonstrate **Haasya** / laughter / comic expression for Maya and the audience.)*

- **Haasya mudras / comedy dances** performed live
- **Song block (TBD):**
  1. __________ *(Haasya Ras song 1 — TBD)*
  2. __________ *(TBD)*
  3. __________ *(TBD)*

**Choreographer:** TBD — assign (Haasya dances + song block)

> Maya still may not laugh on screen — contrast with joyful live stage. Audience gets the Ras even if AI doesn't yet.

**→ NEXT: Cycle Movie spy montage → Scene 4 (gym) → Scene 5 (race).**

---

## Cycle Movie — Spy Escape *(montage before Scene 4)* [VIDEO]

> **Format:** Video — Bollywood-style mini-movie cold open inside the human story. **Karan** = **Jitin**, **Arjun** = **Kalpesh**, **Sister** = **Vandana**.
>
> **Locations:** Karan's home / Livia unit → streets (cycle escape) → arrives at **Livia gym** → **Scene 4**

---

**Beat 1 — Sister vs sports channel** *(Vandana)*

*(Vandana doing household work.)*

**Vandana (Sister):** "Bhaiya… don't keep watching the sports channel… finish your other work too…"

**Karan:** "Arey… aata hoon… important race hai…" *(cycle race on TV)*

**Vandana:** "And that office ka call jo tune bola tha…?"

**Karan:** "O teri… chod de… ab kya fayda…"

**Vandana** *(to herself / aside):* "Oh, he never listens… yeh sports pe peeche hi pada rehta hai… khel khel mein ek din office se nikal denge…"

---

**Beat 2 — CBI "spy" gag**

*(Sound: door knock and doorbell.)*

*(Three **CBI-type** agents enter — dark suits, deadpan.)*

**Agent:** "Your brother doesn't work… because there is a secret… **he is a spy.**"

**Vandana:** *(acts blur — confused, stunned)*

**Agent:** "The nation needs him now — we are taking him immediately."

*(Vandana **faints**.)*

---

**Beat 3 — Arjun rescue + song**

**Arjun and friends:** take Karan and **run away on cycles.**

**Music:** **"Woh Sikandar…"** starts — *"Woh Sikandar Bhi Doston Ke Hain"* (Jo Jeeta Wohi Sikandar energy; chase / buddy escape montage).

```
(TBD — chase route; cycle escape path to Livia gym)
```

---

**SCENE 4 — Gym: Friends & Challenge [VIDEO]**

*(Livia gym / fitness area — friends surround **Karan**. Cast from Team A/B — assign who plays each type.)*

**The Gym Bro:** "Bhai, my bench press has been stuck at the same weight for a month. What should I change in my routine?"

**The Runner:** "Yaar Karan, I want to run a 10k next month but my knees start hurting after 3 kilometers. Give me some tips na!"

**The Cricketer:** "Listen, my timing on the cover drive is completely off lately. Can you come to the nets this weekend and check my stance?"

**The Casual Fitness Friend:** "Bro, I want to start waking up early for yoga but I just can't leave my bed. How do you stay so disciplined?"

**Arjun** *(to all):* "Bhai log, sabka ho jayega… saans toh lene dena usko…"

**Karan:** "Sahi dramebaaz… bahut tight hai kaam… upar se mere behen ka to-do list… sports ke liye time kahaan milta hai.."

**Arjun:** "Chal banate hain… race kar abhi… mere talent mein tu compete nahin kar saktha.. lekin..tere sports mein equally takkar de saktha hoon.."

**Karan:** "Chalo dekthein hein.."

```
(TBD — which performers play Gym Bro / Runner / Cricketer / Casual Friend)
```

**→ Scene 5 — the race.**

---

**SCENE 5 — Race: Karan Wins [VIDEO]**

*(Impromptu race between friends — intro energy, everyone lining up / banter.)*

*(They race — **Karan wins.** Everyone praises Karan. **Arjun is proud** — no jealousy yet; establishes the duo at their best.)*

```
(TBD — foot race vs cycle; outdoor route; race montage shot list; music — continue "Woh Sikandar" or new track)
```

> **Story note:** Comic spy spoof (pre–Scene 4) → buddy energy + Karan as natural leader (Scenes 4–5). Arjun proud here — contrast for later friction (**Scene 10**). **Scene 8** song overlap — "Woh Sikandar" may span escape + race montage.

---

**SCENE 6 — "Yeh Bandhan Toh Pyaar Ka Bandhan Hai" — TRAILER SCENE (not the movie itself)**

*(This is deliberately staged as a mock movie-TRAILER, not a normal scene — think quick cuts, freeze-frames on Karan & Arjun mid-laugh, punchy text cards ("Two Friends." / "One Dream." / "Yeh Bandhan Toh Pyaar Ka Bandhan Hai" as the trailer's theme song under it), maybe a booming trailer-voice narrator. It's Maya playing back an over-the-top, cheesy "coming attractions" reel of Karan & Arjun's friendship — not real-time footage of the actual scene.)*

**PratiKool** *(calls Maya, breaking the fourth wall — now landing literally, since Maya just played an actual trailer):* "Hey, scene change kar — this is old Bollywood style."

**Maya:** "Ok."

*(Cut — out of the trailer, into the real scene: Karan and Arjun are sitting with their friends in a dining setup.)*

*(The friends ask Arjun to help Karan meet Arjun's contacts/friends for sponsorships.)*

```
(type here — who asks, exact lines, and how Arjun responds, and tell the agent to fold them in)
```

> This is a nice early beat for Arjun — the group leans on *his* network/connections to help Karan, giving Arjun real value and standing in the group before anything goes wrong. Makes it sting more later that he still feels sidelined despite this.

---

**SCENE 7 — Meet the Sponsor: Nishanth's Office**

*(Karan and Arjun meet the sponsor, **Nishanth**, in his office. Nishanth introduces them to Coach **Satak Lee**, who will train them for the Triathlon challenge.)*

People involved: **Karan, Arjun, Nishanth, Satak Lee** — all in Nishanth's office.

```
(type here — Nishanth's pitch / the sponsorship terms / how Satak Lee is introduced, and tell the agent to fold them in)
```

---

**SCENE 8 — Sponsorship Approved: Celebration [VIDEO]** ⚠️ *review — "Woh Sikandar" used in spy escape (pre–Scene 4)*

*(Arjun and Karan meet their friends, who have been **waiting outside** to find out if they got sponsorship approval. Good news — they're in. **Karan thanks Arjun in front of everyone.** Team A, Team B, and **Coach Satak Lee** celebrate together.)*

**Music:** **TBD** — was **"Woh Sikandar Bhi Doston Ke Hain"** (now used in Cycle Movie Scene 1 escape); pick reprise, different song, or montage continuation.

People involved: **Karan, Arjun, Team A, Team B, Coach Satak Lee**, friends waiting outside.

**Kool** *(heaven / video overlay):* "This is so my type of feeling… Over to you, Maya."

**Production — assign:**
- **Choreographer:** TBD
- **Locations:** SUTD, swimming pool, tennis court, basketball court (montage / multi-sport training energy)

```
(type here — exact outside-meeting beat / who speaks / montage shot list, and tell the agent to fold them in)
```

> Full scene-flow tracker (people, locations, status for every scene): `livia-navras-scene-flow.md`

---

**SCENE 9 — TBD (bridge beat)**

```
(placeholder — training montage / Naina beat? confirm with team before scripting)
```

---

**SCENE 10 — Arjun & Karan Get Irritated [STAGE]**

*(Arjun and Karan are working out. Phone rings — **Kool picks up and walks away.** Arjun **shouts at Karan in front of everyone** about discipline. Karan feels insulted. **Coach Satak Lee** is watching. **Arjun storms out** — talking to **PratiKool** as he goes; **Coach overhears**.)*

**Kool** *(to Arjun):* "Bhai its ok na.. kar raha hai mehnat.. don't say such things na.. demotivate karogey.."

**Kool** *(to Ankit & Coach):* "Can you take him away and give a break.."

**PratiKool** *(to Karan):* "Arey, bahut advice de raha hai Arjun aajkal but khud toh kuch nahin kar raha hai.. yeh iska race bhi toh hai.."

*(Karan nods.)*

**PratiKool** *(to Pramod):* "Bol na.. sirf mein bolun kya.."

**Pramod:** "I meant agar thoda masti ched-chaat kar diya to kya ho jayega..? Bade followers hai to hum pe bhi influence karega."

**Outside — Kool to Coach:** "Lets get the team together — we need some party fun and entertainment, that will bring them together. Lets do it with **Arjun's show** itself — he'll be easier to talk to after a good show when his mood is good."

*(All friends say it's a good plan.)*

**People:** Karan, Arjun, Kool, PratiKool, Coach Satak Lee, Ankit, Pramod, Team A/B  
**Location:** TBD — **pool area or gym (24-hr fitness)**

---

**YOUR NOTES — scenes 1–10** *(Scene 11+ TBD — user adding new beat tomorrow)*

---

## Running order (synced from Miro "Step-by-Step Kanban", 44 steps)

> Each step below has placeholders for **Characters** and **Music** — same placeholders as the Miro cards. Fill in either place; tell the agent to sync and it'll reconcile both directions.

### SETUP / INTRO — Why Maya exists

1. **Karma Dev's Intro — see "Opening Scene — Full Draft" above for the full dialogue.** Karma Dev, Kool & PratiKool (no Audit Devi) visit AKV; Chitragupt's overload is the real trigger. · Characters: Karma Dev, Kool, PratiKool, AKV, Maya · Music: Karma hook sting + "Navras Se Hai Kalpana" title song

   **🎬 Filmed as the OPENING VIDEO (0:00–~2:07, runtime TBD)** — this whole step is a pre-show/opening video, not staged live. Shots 4.1–5B.2 (crisis beats) **CUT per Option A** — see the full note under "Opening Scene — Full Draft" above. Shot 3.3 still needs its character swap (Audit Devi → Maya), flagged in the shotlist CSV/Miro table.
   - **0:00–0:20 Earth Entry [generic]** — Singapore satellite view at night → camera rises through the atmosphere → Earth tilts, Indian subcontinent glows gold. No characters yet.
   - **0:20–0:45 Cosmic Journey [generic]** — flight through star fields, a crystalline geometric galaxy, cosmic nodes connected by light. Still no named characters — pure atmosphere/world-building.
   - **0:45–1:10 Heaven Approach [generic, with a tease]** — glowing temple architecture, light-gates; final shot (1:05–1:10) hints at four glowing silhouettes inside the Karma Sphere — not yet identifiable.
   - **1:10–1:35 Karma Colony Arrival — CHARACTER REVEAL** — the camera arrives at the Karma Station and the four real characters are revealed one by one: Karma Dev (gold/saffron, ledger in hand, showman pose) → Audit Devi (cool blue-white, floating ledger-tablet) → Kool (warm positive energy trail) → PratiKool (dark red energy, mirrored opposite Kool). All four assembled at the portal by 1:35.
   - **1:35–2:07 Interior — The Four at Work → The Realization** — Karma Dev directs, Audit Devi audits, Kool/PratiKool push opposite energy streams; karma orbs multiply faster than they can sort. Audit Devi flags the overload; Karma Dev decides they need help, gesturing toward a vision of AKV's lab. **Final shot (2:03–2:07): all four turn to face the audience in a held pose — video fades to white exactly as stage lights snap up on the live actors in the same pose/costume**, so it looks like they stepped out of the screen. This is the live handoff into Scene 2.
   - **Costume rule:** each character's video glow color must match their stage costume exactly (gold = Karma Dev, blue-white = Audit Devi, warm gold-green = Kool, deep red = PratiKool) so the handoff reads instantly.
   - Full shot-by-shot breakdown + AI video prompts: `livia-navras-intro-video-shotlist.csv` (also on Miro as the "Video Production — Opening Video" Kanban).
2. **AKV's Creation Video — now the cold open of the Opening Scene above** (not a separate step). AKV switches Maya on; she's brilliant, empty — "Emotions: not found." (Soft workshop underscore.) · Characters: ________ · Music: ________
3. **AI Training Montage** — Maya learns handshakes, jumps, dance, talk — mechanically, comically. Still happens, as part of the same opening screen sequence, before Karma Dev's team arrives. · Characters: ________ · Music: ________
4. **Maya Watches Kids Play** — she's confused by their laughter and games. Still happens, same as above. · Characters: ________ · Music: ________
5. **SUPERSEDED — Audit Devi's Complaint.** Audit Devi is cut from the cast. The "Maya can't understand emotions" beat is now delivered by AKV + PratiKool's "deal breaker" line inside the Opening Scene above.
6. **MERGED into the Opening Scene above — Karma Proposes the Navras.** Karma's couplet ("Man mein jo hai bhavna...") + his Natya Shastra pitch now closes that scene directly, rather than being a separate step.
7. **Introducing the Livia Team — see "Scene 2 — In Heaven" above for the full dialogue.** Maya, still confused, is sent by Karma to do on-the-job training: track Kool & PratiKool (left behind on Earth) via a Google Earth zoom into Livia. · Characters: Karma Dev, Maya, Kool, PratiKool (referenced) · Music: ________
8. **Cycle Movie — Spy Escape** — see spy montage above [VIDEO]. Vandana (sister) scolds Karan; CBI spy gag → escape on cycles (**"Woh Sikandar"**). · Characters: Karan, Vandana, Arjun · Music: "Woh Sikandar…"
9. **Gym: Friends & Challenge — see "Scene 4" above [VIDEO].** Friends ask Karan for fitness tips → Arjun redirects → race challenge. · Characters: Karan, Arjun, friends · Music: ________
10. **Race: Karan Wins — see "Scene 5" above [VIDEO].** Impromptu race → Karan wins, all praise, Arjun proud. Then **"Yeh Bandhan"** trailer (Scene 6). · Characters: Karan, Arjun, friends · Music: TBD
11. **The Sponsorship Challenge — see "Scene 7" above.** · Characters: Karan, Arjun, Nishanth, Satak Lee · Music: "Aata Maji Satak Li"
12. **Sponsorship Approved — see "Scene 8" above [VIDEO].** Friends waiting outside; Karan thanks Arjun in front of all; Team A, Team B, Coach celebrate. Kool: "This is so my type of feeling… Over to you, Maya." · Characters: Karan, Arjun, Team A, Team B, Satak Lee, Kool, Maya · Music: TBD · **Choreographer: TBD** · **Locations: SUTD, swimming pool, tennis court, basketball court**
13. **Training Begins** — *[may overlap Scene 8 montage — confirm with team]* · Characters: ________ · Music: ________
14. **Explaining the Stakes** — *[partially covered in Scene 8 celebration — confirm merge]* · Characters: ________ · Music: ________
15. **"All Is Well" Montage (Video)** — *[may merge with Scene 8 — confirm]* · Characters: ________ · Music: ________
13. **Karma's Aside: Achieving Isn't Easy** — Karma congratulates the AI's start, warns that achieving a goal is never easy. *[Needs staging expansion.]* · Characters: ________ · Music: ________
14. **Basketball vs Girls' Team** — Naina enters, boys tease Karan ("tere naina..."), he's distracted — "Yeh Ladka Hai Deewana." Arjun feels sidelined. · Characters: ________ · Music: ________
15. **The Fork** — PratiKool instigates Arjun's jealousy; Kool nudges Karan toward Naina (KKHH sting → Main Hoon Na). · Characters: ________ · Music: ________
18. **Kool Pitches a Party to Coach** — sees a drift in the team, suggests a party to re-motivate them. · Characters: ________ · Music: ________
28. **Karma & Audit Devi's Commentary** — reflecting on how negative emotion turns a life — "Jaisi Karni, Waisi Bharni." · Characters: ________ · Music: ________
34. **Karma's Transition to Bhayanak** — "there's more to learn... two spectrums, negative and positive... you'll have to go back before humans even evolved." · Characters: ________ · Music: ________

### SHRINGAR (LOVE)

16. **Karma's First Shringar Lesson (Video)** — Karma's first Shringar introduction to the AI begins here. · Characters: ________ · Music: ________
17. **Shringar: Karan & Naina** — their spark ("Tumhe Jo Maine Dekha"). · Characters: ________ · Music: ________
18. **Shringar Garden backdrop video** — 2-minute moonlit Indian mythological garden plate for song-block green-screen compositing. **20 Google Flow clips** — shot tracker: `livia-navras-shringar-garden-shotlist.csv` · full prompts in **Scene Shringar Garden** section above. Source storyboard: `Dilse 2026/Livia Talkies 6/Story/1st Ras/Shringar Garden/garden_video_storyboard.md` · Characters: none (no humans) · Music: ambient garden bed under vocals

### HASYA (LAUGHTER)

19. **Karma Introduces Hasya Ras** — Heaven-set stage scene: Kool & PratiKool crack bad jokes (a "don't laugh" challenge) — only Audit Devi & Karma laugh; Maya gives horrible emoticons. Kool to PratiKool: "AI mein tera influence zyaada lag raha hai." · Characters: ________ · Music: ________
20. **Hasya: The Qawwali Party** — joyful chaos; Kool awards bonus points, PratiKool docks them. · Characters: ________ · Music: ________
21. **Songs & Dances on Stage** — *[placeholder, to be expanded with specific numbers]*. · Characters: ________ · Music: ________

### BIBHATSA (DISGUST)

22. **Jealousy Surfaces** — staged live, continues on screen as the song sequence finishes. · Characters: ________ · Music: ________
23. **Bibhatsa: Arjun Plots Karan's Disqualification** — Arjun schemes to get Karan disqualified, wants to be sole leader, self-justifies: "I'm not distracted by girls." · Characters: ________ · Music: ________
24. **PratiKool's Bad Ideas** — PratiKool feeds Arjun a plan; Arjun notes their morning cycling ritual: "now I will teach a lesson there itself." · Characters: ________ · Music: ________
25. **The Sabotage (Video)** — during "Hawa Ke Saath Saath," Arjun rigs an invisible string; Karan falls, badly hurt — the cycle damaged too, unnoticed. (Karuna music starts.) · Characters: ________ · Music: ________

### KARUNA (SORROW)

26. **Karuna: The Accident's Aftermath** — friends rush to Karan, notice Arjun's absence; Kool suspects his guilt. Coach says he must pull Karan from the contract unless he recovers in time. · Characters: ________ · Music: ________

### RAUDRA (ANGER)

27. **Raudra: Friends Are Furious** — the team is furious; Karan storms out to confront and hurt Arjun (and his cycle) — Durandhar-style rage scene. *[To be built.]* · Characters: ________ · Music: ________
29. **Coach & Kool Intervene** — they stop Karan from destroying himself and Arjun; redirect toward sportsmanship. · Characters: ________ · Music: ________

### VEERA (COURAGE)

30. **Veera: Kool Redirects the Anger** — (Veer Ras song, background/stage.) Gym/swim/run recovery support; the team reconciles. · Characters: ________ · Music: ________
31. **Arjun's Guilt & Isolation** — Arjun watches Karan train, feels guilty and lonely — only the coach still works with him. · Characters: ________ · Music: ________
32. **Race Day** — Arjun, the true leader, struggles on his old, damaged cycle and trips/falls as it gives way beneath him. Karan sees this, stops, and says: "You take this.. finish it" — handing Arjun his own cycle and giving up his own shot at the win. Arjun finishes the race on Karan's cycle, humbled by the friend he wronged. · Characters: ________ · Music: ________
33. **Coach's Verdict** — sportsmanship and team spirit are the real win; recommends both for sponsorship. · Characters: ________ · Music: ________

### BHAYANAKA (FEAR)

35. **Bhayanak Ras on Stage** — 9 Devis, Kaali, Shiv. · Characters: ________ · Music: ________

### SHANTA (PEACE)

36. **Shanti Ras** — *[placeholder, not yet detailed]*. · Characters: ________ · Music: ________

### ADBHUTA (WONDER) — FINALE

37. **Adbhuta Ras** — Karma (video): humans can choose the Bhayanak side or move toward Shanti. Maya: "I understand now that being human and managing this emotion itself is the Adbhut Ras." · Characters: ________ · Music: ________
38. **Finale: Karma and Emote 2.0** — closing song. Note: these same rasas, unchecked, can also pull humans toward "Rakshash" level — the other side of the same coin. · Characters: ________ · Music: ________

---

### PARKED / ALT IDEAS (not in the confirmed sequence)

- **ALT — Veera: Condo Sports Day** — condo B-team comic sports montage.
- **ALT — Bhayanaka: Haunted Lift Prank** — kids scare an elder, spooky-fun garba/dandiya.
- **ALT — Bhayanaka: Hospital Wait & Kaali Vision** — Kaali/Kaal vision during a hospital wait.
- **ALT — Karuna: Too Much World** — news montage, contemporary dance, global sorrow.
- **ALT — Bibhatsa: Modern Draupadi** — modernized gossip-shaming story.
- **ALT — Hasya: Ek Chatur Nar Shayari Contest** — shayari contest with dancers.

**YOUR NOTES — Running order**

```
(type here — reorder, add missing beats, or flag anything that should change)
```

---

## Opening video — AI prompts log (Google Flow / Veo)

> Paste each finalized prompt here as you generate it, plus which take you kept. Keeps the shotlist CSV as the plan and this as the "what actually worked" record.

**Shot 1.1 (0:00–0:05, Earth Entry)** — status: ✅ generated & kept

```
Cinematic aerial drone shot at night over Singapore's Marina Bay skyline — Marina Bay Sands, the ArtScience Museum, and the CBD skyscrapers glittering with warm gold city lights against a deep indigo night sky. Camera holds high and steady above the bay, then begins a slow, almost imperceptible push-in toward the city center. Thin wisps of cloud drift through the foreground. City lights shimmer in reflection on the dark water of the bay. A faint, warm golden aura begins to glow softly along the horizon behind the skyline — like something vast is quietly waking up beyond the sky.

Style: ultra-realistic aerial/satellite cinematography, high detail, cool blue-black color grade with warm gold highlights, subtle film grain, no text overlays, no people visible.
Mood: quiet, vast, reverent, anticipatory — the calm before the film opens up into the cosmos.
Camera: slow smooth push-in, minimal shake, no cuts.
Audio: low ambient drone tone, faint deep pulse building underneath, no dialogue, no music sting yet.
Duration: 5 seconds. Aspect ratio: 16:9.
```

**Shot 1.2 (0:05–0:15, Earth Entry)** — status: ✅ generated & kept

```
Continuing directly from the previous shot of Singapore's skyline at night: the camera now begins a smooth, powerful vertical rise, ascending straight up and away from the city. As it climbs, the skyline shrinks below, city lights blur into a glowing golden patch, and thin clouds streak past the lens. The camera breaks through wispy cloud layers into clear night air, then keeps rising until the curvature of the Earth begins to appear at the edges of frame. Faint particle effects — like glowing embers or motes of light — drift upward from the city below, as if energy itself is rising off the earth. The sky deepens from indigo toward near-black, stars beginning to appear. Warm gold light trails linger from the city below, with a hint of purple bleeding in at the edges of frame as we near the atmosphere's edge.

Style: ultra-realistic, cinematic aerial-to-orbital transition, seamless continuous camera movement matching the previous shot's grade (cool blue-black with warm gold highlights), subtle film grain, no text, no people.
Mood: awe-inspiring, escalating — a sense of ascension and scale.
Camera: continuous smooth vertical rise, slight deceleration near the top, no cuts.
Audio: ambient drone swells, soft building instrumental undertone, airy whoosh as we pass through cloud layers, no dialogue.
Duration: 10 seconds. Aspect ratio: 16:9.
```

**Shot 1.3 (0:15–0:20, Earth Entry)** — status: drafted, not yet generated

```
Continuing from orbit: the camera now holds a high, stable orbital view of Earth, curved and glowing against the blackness of space, thin atmospheric haze rimming the horizon. Slowly, smoothly, the Earth tilts/rotates beneath the camera until the Indian subcontinent rotates into full view. As it centers in frame, the landmass begins to glow with a warm golden light emanating from within — as if lit from inside rather than reflecting the sun. Fine, glowing thread-like lines (ley lines) bloom across the surface of the subcontinent, connecting in delicate geometric patterns, pulsing gently like a living circuit. Stars are sharp and still in the background. In the final second, the frame begins to desaturate at the edges and fade — the Earth and its golden glow receding as if being pulled backward into darkness, transitioning into deep space.

Style: ultra-realistic orbital/space cinematography blending into a subtle holographic overlay effect for the ley lines, cool black-and-blue space backdrop, warm gold glow as the sole warm accent, no text, no people.
Mood: revelation, sacred, quietly momentous — this is the first sign that something ancient and vast lies beneath the ordinary world.
Camera: slow stable orbital hold, gentle push-out at the very end to begin the transition to cosmic space.
Audio: ambient drone swells further, a single soft resonant tone (like a struck bell or singing bowl) as the ley lines bloom, fading into a lower cosmic rumble at the very end.
Duration: 5 seconds. Aspect ratio: 16:9.
```

**Shot 1.25 (0:12–0:16, Earth Entry, bridging shot)** — status: in progress — generated as a zoom-IN, to be reversed in editor

*(Fix for the jarring 1.2→1.3 cut. Generate a zoom-IN toward Singapore through golden dust — camera approaching, dust converging/thickening toward the city, city growing from a small dot into full view — then reverse the clip in your editor so it plays as a smooth zoom-OUT with dust dispersing and the city settling back into a small dot. Reuse Shot 1.1's Singapore skyline framing as the "arrival" point so the reversed ending matches Shot 1.2's starting look.)*

---

## Scene 2 — Cosmic Journey (0:20–0:45) — generic, no characters yet

**Shot 2.1 (0:20–0:28)** — status: drafted, not yet generated

```
Continuing directly from the previous shot's fade into deep space: the camera now flies forward smoothly through a vast star field, stars streaking past on either side with gentle parallax — closer stars blurring past quickly, distant stars barely moving. Fine cosmic dust and faint nebula haze drift through the frame, catching soft light. Ahead in the far distance, a glowing galaxy begins to emerge from the darkness, small at first, its light barely visible. The color palette shifts from the cool black-blue of near space into deeper, richer purples and electric blues, with points of starlight in warm gold and white.

Style: ultra-realistic deep-space cinematography with a painterly, dreamlike edge; multiple depth layers of stars and dust for strong parallax; no text, no people, no spacecraft.
Mood: expansive, otherworldly, quietly building anticipation.
Camera: smooth continuous forward dolly/push through space, constant unhurried speed, no cuts.
Audio: ambient drone continues building in intensity and pitch from the previous shot, faint shimmering high textures entering, still no dialogue, no full instrumentation yet.
Duration: 8 seconds. Aspect ratio: 16:9.
```

**Shot 2.2 (0:28–0:35)** — status: drafted, not yet generated

```
Continuing forward toward the galaxy now revealed as a crystalline, geometric structure rather than a traditional spiral — faceted like a cut gemstone, its vertices glowing with inner light and connected by fine geometric lines echoing sacred-geometry patterns (hexagons, interlocking triangles, mandala-like symmetry). The camera arcs into a smooth 360-degree orbit around this crystalline galaxy, revealing new facets and angles as it circles. Light refracts and scatters through the crystal surfaces in shifting colors — deep purple, electric blue, and flashes of gold — casting prismatic light trails across the surrounding darkness. Fine cosmic dust drifts past in the foreground for depth.

Style: ultra-realistic deep space rendering merged with stylized sacred-geometry crystal design, prismatic light refraction effects, rich purple/blue/gold palette, no text, no people.
Mood: awe, intricate wonder — something intelligent and ancient built this.
Camera: smooth continuous 360-degree orbital arc around the crystalline structure, constant speed, no cuts, no shake.
Audio: ambient drone continues building, joined by soft crystalline/bell-like tones that shimmer in sync with the light refractions, tempo beginning to pick up slightly.
Duration: 7 seconds. Aspect ratio: 16:9.
```

**Shot 2.3 (0:35–0:45)** — status: drafted, not yet generated

```
Continuing the journey: the camera now weaves smoothly between three or four additional cosmic structures, each visually distinct — one a softly pulsing sphere of golden light, one a slow-spinning geometric lattice, one a cluster of glowing orbs linked by threads of light — all connected to each other and to the crystalline galaxy behind by fine glowing data-stream lines that pulse with traveling light, like a vast cosmic network. As the camera weaves past the last of these structures, the space ahead begins to open up and brighten, revealing the edge of a much larger, warmer glow in the far distance — golden and vast, unlike the cool blues and purples around it — hinting at what lies ahead. The camera's path curves gently to center on this new light as the shot ends.

Style: ultra-realistic deep-space cinematography merged with stylized geometric/mystical structures, glowing data-stream connections, palette shifting from cool purple/blue toward warm gold as the shot progresses, no text, no people.
Mood: building momentum and wonder, a sense of arriving somewhere significant.
Camera: smooth continuous weaving path between the structures, gently curving to settle on the distant golden glow by the end, no cuts.
Audio: ambient drone and crystalline tones continue building into a fuller, more orchestral swell; tempo increasing; a rising sense of arrival.
Duration: 10 seconds — if your Flow tier caps single-clip length below this, split into two ~5s beats back-to-back (weave, then curve-to-settle) and stitch in the editor. Aspect ratio: 16:9.
```

---

## Scene 3 — Heaven Approach (0:45–1:10) — generic, first tease of the 4 characters at the end

**Shot 3.1 (0:45–0:55)** — status: drafted, not yet generated

```
Continuing directly from the previous shot's approach toward the warm golden glow in the distance: the camera keeps flying steadily forward, and that glow now resolves into the silhouette of a massive structure ahead — towering spires, domes, and palace rooftops, all impossibly tall, built entirely of shimmering golden-white light rather than solid material. Wisps of luminous cloud drift across the silhouette, and rays of warm golden light break through the cloud in dramatic shafts. As the camera draws closer, intricate carved details begin to resolve on the surfaces of the towers — lotus motifs, mandala patterns, temple-spire finials — all glowing faintly from within. The overall palette shifts from the cool purples/blues of deep space into warm golden yellows, soft celestial whites, with cosmic purple still lingering at the edges of the frame.

Style: ultra-realistic cinematic architecture rendering combined with divine/mythic lighting; ancient Indian temple and palace design language (shikhara spires, domes, intricate stone carving) reimagined as structures of pure light; volumetric god-rays; no text, no people, no characters yet.
Mood: awe, arrival, sacred — the audience is entering somewhere ancient and vast.
Camera: continuous smooth forward flight, gently decelerating as the structure fills more of frame, no cuts.
Audio: the ambient orchestral swell from the previous shot continues and grows fuller, joined by soft sustained choir-like tones and a distant temple bell, no dialogue.
Duration: 8 seconds. Aspect ratio: 16:9.
```

**Shot 3.2 (0:55–1:05)** — status: drafted, not yet generated

```
Continuing forward: the camera now passes directly through a massive glowing archway/gate at the entrance of the light-structure, the gateway briefly filling the frame with warm white light as the camera crosses the threshold, before opening into a vast interior realm of palaces, floating courtyards, and glowing walkways connected by beams of light. Divine birds — luminous, trailing soft light from their wingtips — glide across the middle distance. Faint, translucent celestial figures are visible far in the background, tending to unseen tasks, indistinct and glowing. Volumetric light pours down from above in soft golden shafts, and fine particles of light drift downward through the air like slow luminous rain. In the far distance, past the palaces, a massive spherical structure begins to become visible for the first time — glowing, geometric, unmistakably different in design from the organic palace architecture around it: the edge of the Karma Sphere.

Style: ultra-realistic + mythic architectural fantasy rendering, divine volumetric lighting, soft luminous particle effects, warm gold/white/celestial-purple palette, no text, no readable characters yet — the Karma Sphere should read as a distant, mysterious shape only.
Mood: wonder deepening into curiosity — something at the heart of this realm is different from the rest of it.
Camera: continuous smooth forward flight through the archway and into the interior, gently arcing to reveal the Karma Sphere in the distance by the end of the shot, no cuts.
Audio: choir tones and orchestral swell continue building, a soft rhythmic pulse begins underneath (like a distant heartbeat), hinting at the mechanical/energetic nature of what's ahead.
Duration: 10 seconds — if capped, split into a 5s threshold-crossing beat and a 5s interior-reveal beat. Aspect ratio: 16:9.
```

**Shot 3.3 (1:05–1:10)** — status: drafted, not yet generated — carries the 4-character color tease, watch this one closely

```
The camera now holds in a wide, slow orbital drift, revealing the full scale of the heavenly realm: temples, palaces, and floating platforms connected by chains of pure light, stretching into the distance, with the massive geometric Karma Sphere now clearly visible as the dominant structure at the center of it all, glowing steadily. As the camera drifts, it slowly pushes toward the Karma Sphere. Within the Sphere's glow, near its base or at an entrance point, four indistinct silhouettes can just be made out — tall figures standing together, each haloed in a different faint color that is not yet fully revealed: one edged in warm gold, one in cool blue-white, one in a soft warm green-gold, and one in a deep, smoldering red. They are too distant and too softly lit to make out any detail — just four presences, waiting.

Style: ultra-realistic mythic-architectural wide shot, grand scale conveyed through size and distance relationships, four silhouettes rendered as soft glowing outlines only — no facial or costume detail, just color-coded auras. No text.
Mood: quiet anticipation — something (someone) is waiting at the heart of all this light.
Camera: slow orbital drift combined with a gentle push toward the Karma Sphere, ending on the four silhouettes small in frame, no cuts.
Audio: the orchestral swell and pulse reach a held, suspended note — quieter, more intimate — as if the music itself is holding its breath before the reveal.
Duration: 5 seconds. Aspect ratio: 16:9.
```

---

## Karma Chakra — green-screen motion library (Google Flow / Veo)

> **Asset:** ornate Indian mythological celestial chariot **"Karma Chakra"** — gold filigree, stone balustrade, pink lotus flowers, Om symbol and nameplate at front, elevated blue throne under a golden chhatri canopy at rear, four glowing cosmic portal wheels (swirling blue/purple/white energy) instead of tires, golden lightning crawling across the hull, soft purple/blue cosmic aura hugging the vehicle only.
>
> **Purpose:** generate short green-screen clips for compositing — fly the chariot over cosmic backgrounds, heaven plates, etc.
>
> **Workflow:** Google Flow → **Image to Video** → upload the green-screen reference still → paste prompt → **16:9**, **10s** (split into two ~5s beats and stitch if your tier caps length).
>
> **Green-screen rules (critical):** background stays flat, uniform chroma green for the full clip — no floor, horizon, sky, stars, or environment; no shadows on green; only the chariot + its immediate glow/particles move.

| Style | Camera | Chariot motion | Best for compositing as… |
|-------|--------|----------------|--------------------------|
| Zoom & Pulse | Push-in to nameplate | Wheels pulse, lightning flickers | Hero intro / title moment |
| Wave | Locked off | Gentle bob + roll | Floating through nebula plates |
| Cosmic Spin | Locked off | Full 360° yaw | Reveal / transition wipe |
| Dive & Rise | Follows vertical arc | Nose-down dive → power rise | Launch / warp-speed entry |
| Orbital | 180° orbit | Subtle hover animation | Beauty shot over cosmic BG |

**Compositing notes:** key out green in Premiere/CapCut/DaVinci; add cosmic BG plate underneath; if green bleeds on gold edges, shrink the key or add a soft matte; if Flow garbles the nameplate text, prefer Wave or Orbital takes (less zoom on text) or paint a still over it.

**Karma Chakra — Zoom & Pulse** — status: drafted, not yet generated

```
Image-to-video of the Karma Chakra celestial chariot on a solid chroma-key green screen background.

The ornate Indian mythological chariot holds perfectly centered in frame — gold filigree, pink lotus flowers, Om nameplate at the front reading "karma chakra," elevated blue throne under a golden chhatri canopy at the rear, four glowing cosmic portal wheels of swirling blue-purple-white energy beneath it, golden electricity crawling across the hull, soft purple-blue aura hugging the vehicle only.

Motion: the chariot stays mostly stationary in space while its cosmic wheels pulse rhythmically — swelling brighter then dimming in a slow heartbeat (two pulses across the clip). Golden lightning flickers across the filigree in sync with each pulse. Fine cosmic sparkles drift outward from the wheels and dissolve before reaching the green background.

Style: ultra-detailed mythic-fantasy 3D render, rich gold/blue/purple palette, sharp chariot detail, perfectly flat uniform green screen behind — no environment, no floor, no shadows on green.
Mood: divine, powerful, charged — like a cosmic engine warming up.
Camera: slow smooth push-in (zoom) toward the front nameplate and Om symbol, starting medium-wide and ending in a medium close-up of the front third of the chariot. No cuts, no shake.
Audio: low cosmic hum with a soft rhythmic pulse matching the wheel glow, faint electrical crackle, no dialogue.
Duration: 10 seconds. Aspect ratio: 16:9. Background must remain solid green throughout.
```

**Karma Chakra — Wave** — status: drafted, not yet generated

```
Image-to-video of the Karma Chakra celestial chariot on a solid chroma-key green screen background.

The ornate Indian mythological chariot — gold filigree, stone balustrade, pink lotus flowers, Om nameplate, blue throne under golden chhatri canopy, four swirling cosmic energy portal wheels — floats in empty green-screen space.

Motion: the entire chariot moves in a smooth, graceful wave pattern — gentle rise and fall like riding an invisible cosmic swell, with a subtle slow roll (2–3 degrees) left then right. The cosmic wheels spin continuously, trailing soft blue light ribbons that curl and fade. Golden electricity ripples along the hull in flowing lines, following the wave rhythm. Pink lotus petals at the front flutter slightly.

Style: ultra-detailed mythic-fantasy 3D render, cinematic lighting on the chariot only, perfectly flat uniform chroma green behind — no water, no clouds, no environment, no floor.
Mood: majestic, fluid, serene power — a divine vehicle gliding through the void.
Camera: locked-off medium shot, chariot centered; all movement comes from the chariot's wave motion, not the camera. No cuts.
Audio: soft whooshing air, gentle cosmic swell, distant harmonic drone, no dialogue.
Duration: 10 seconds. Aspect ratio: 16:9. Background must remain solid green throughout.
```

**Karma Chakra — Cosmic Spin** — status: drafted, not yet generated

```
Image-to-video of the Karma Chakra celestial chariot on a solid chroma-key green screen background.

The ornate Indian mythological chariot — gold filigree, pink lotus flowers, Om nameplate, blue throne under golden chhatri canopy, four glowing cosmic portal wheels — hovers in green-screen void.

Motion: the chariot performs one slow, majestic 360-degree yaw rotation (spinning horizontally on its vertical axis) over the full 10 seconds — revealing front, side profile, rear throne, opposite side, and back to front. The cosmic wheels spin faster during the turn, throwing off spiraling blue-purple energy trails and stardust that orbit the hull briefly then dissipate. Golden lightning intensifies along the filigree during the spin. The chhatri canopy fringe sways gently from the rotational energy.

Style: ultra-detailed mythic-fantasy 3D render, dramatic rim lighting that shifts as the chariot turns, perfectly flat uniform chroma green behind — no environment, no floor, no shadows on green.
Mood: awe, revelation — showing off the vehicle from every angle like a divine reveal.
Camera: locked-off, chariot stays centered in frame while it spins; camera does not move. No cuts.
Audio: rising cosmic whoosh building through the spin, crystalline shimmer at the halfway point, soft landing tone as it faces front again, no dialogue.
Duration: 10 seconds. Aspect ratio: 16:9. Background must remain solid green throughout.
```

**Karma Chakra — Dive & Rise** — status: drafted, not yet generated

```
Image-to-video of the Karma Chakra celestial chariot on a solid chroma-key green screen background.

The ornate Indian mythological chariot — gold filigree, pink lotus flowers, Om nameplate at front, blue throne under golden chhatri canopy at rear, four swirling cosmic portal wheels — floats in empty green-screen space.

Motion: the chariot executes a smooth dive-and-rise arc over 10 seconds — first tilting nose-down and descending gracefully (first 4 seconds), holding a brief moment at the lowest point with wheels flaring bright blue (seconds 4–5), then sweeping upward in a powerful rise, ending slightly above the starting position with a subtle backward pitch like pulling out of a dive (seconds 5–10). Cosmic wheels blast brighter blue light beams during the rise. Golden electricity streaks backward off the hull like thrust trails. Cosmic particles stream behind during the ascent.

Style: ultra-detailed mythic-fantasy 3D render, dynamic lighting shift on the chariot as it moves, perfectly flat uniform chroma green behind — no sky, no stars, no environment, no floor.
Mood: thrilling, heroic — a divine vehicle launching into the cosmos.
Camera: slow follow movement tracking the chariot's vertical arc — gentle tilt down during the dive, tilt up during the rise — keeping the chariot centered. No cuts, smooth continuous motion.
Audio: deep whoosh on the dive, brief silence/hold at the bottom, powerful ascending cosmic surge on the rise, no dialogue.
Duration: 10 seconds. Aspect ratio: 16:9. Background must remain solid green throughout.
```

**Karma Chakra — Orbital** — status: drafted, not yet generated

```
Image-to-video of the Karma Chakra celestial chariot on a solid chroma-key green screen background.

The ornate Indian mythological chariot — gold filigree, stone balustrade, pink lotus flowers, Om nameplate, elevated blue throne under golden chhatri canopy, four glowing cosmic portal wheels — hovers in green-screen void.

Motion: the chariot holds a steady hover with subtle internal animation only — cosmic wheels spin slowly, golden electricity crawls across filigree, soft purple-blue aura pulses gently. Pink lotus petals tremble slightly. All primary motion comes from the camera orbiting the chariot.

Style: ultra-detailed mythic-fantasy 3D render, lighting shifts naturally as the camera circles, perfectly flat uniform chroma green behind — no environment, no floor, no shadows on green.
Mood: grand, showcase, reverent — a full beauty pass of a mythic vehicle.
Camera: smooth 180-degree orbital arc around the chariot over 10 seconds — starting from the existing three-quarter front angle, circling to reveal the side profile with the throne and chhatri canopy, ending on a rear three-quarter view. Constant speed, no cuts, no shake. Chariot stays centered.
Audio: ambient cosmic drone, soft wheel hum, faint golden electrical texture, no dialogue.
Duration: 10 seconds. Aspect ratio: 16:9. Background must remain solid green throughout.
```

**YOUR NOTES — Karma Chakra takes**

```
(type here — paste kept takes, note what worked/didn't, flag any nameplate text drift)
```

---

## Scene 4 — Karma Colony Arrival (1:10–1:35) — REVISED: robots at work, then the 4 walk in

> Changed from the original "silhouettes solidify in place" idea: now robots are already laboring and struggling when the camera arrives, and Karma Dev, Audit Devi, Kool & PratiKool walk in together to witness it — stronger contrast, and it matches the story logic (an overworked robot workforce is *why* they need Maya).

**Shot 4.1 (1:10–1:16) — robots at work, establishing** — status: drafted, not yet generated

```
Continuing from the previous shot's push toward the Karma Sphere: the camera now passes through the entrance and into a vast interior chamber, geometric and glowing, filled with multiple tiered platforms rising up on all sides. Robots are at work everywhere — golden multi-armed humanoid robots carefully sorting glowing karma orbs by hand, geometric mandala-shaped drones hovering and rotating as they scan streams of orbs, and long serpentine data-bots gliding along channels of light between stations, carrying energy from one platform to the next. The chamber hums with constant, purposeful motion — dozens of robots working in parallel, orbs of light flowing everywhere between them along glowing threads.

Style: ultra-realistic sci-fi/mythic interior rendering, intricate sacred-geometry architecture merged with high-tech machinery, cool electric blues and purples as the base palette with warm gold light from the orbs and data-streams, no text, no named characters yet.
Mood: awe at the scale and precision of it — an entire civilization's worth of quiet, relentless labor.
Camera: continuous smooth forward push into the chamber, then a slow rise to reveal the tiered scale, no cuts.
Audio: the choir/orchestral swell settles into a steady, mechanical-yet-musical rhythm — like a vast engine with a heartbeat — soft metallic chimes layered over the hum.
Duration: 6 seconds. Aspect ratio: 16:9.
```

**Shot 4.2 (1:16–1:22) — the overload becomes visible** — status: drafted, not yet generated

```
Continuing in the same chamber: the camera moves closer to one platform, where the volume of incoming karma orbs is visibly too much — orbs are piling up in overflowing heaps faster than the robots can sort them. One humanoid robot fumbles, and a cascade of glowing orbs spills from its arms and rolls off the platform, scattering. Nearby, a mandala drone's rotation stutters and glitches, its projected light-patterns flickering out of sync. Warning lights — a harsher red-orange — begin to pulse at the edges of a few stations, cutting through the otherwise warm/cool palette.

Style: same ultra-realistic sci-fi/mythic interior style as the previous shot, now introducing visible strain: glitch effects, stuttering light, scattered orbs, red-orange warning accents breaking the calm blue/gold palette.
Mood: shifting from awe to concern — something here is breaking under pressure.
Camera: smooth push-in toward the struggling station, holding briefly on the spilling orbs and the glitching drone, no cuts.
Audio: the steady rhythmic hum starts to stutter and skip, a dissonant note creeps in, an alarm-like chime pulses faintly.
Duration: 6 seconds. Aspect ratio: 16:9.
```

**Shot 4.3 (1:22–1:30) — the four walk in** — status: drafted, not yet generated — key character-introduction shot

```
At the far end of the chamber, a massive archway floods with bright light, and four silhouetted figures step through together, walking forward in a slow, deliberate hero-walk, side by side, unhurried despite the chaos around them. Each figure is haloed in a distinct color, exactly as glimpsed earlier: the central figure glows warm gold, carrying what looks like a glowing ledger or abacus-like instrument; beside him, a figure glows cool blue-white, holding a floating translucent tablet; to one side, a figure glows warm gold-green, moving with a light, flowing energy; on the other side, a figure glows deep smoldering red, moving with sharper, more angular energy, mirroring the gold-green figure's stance. As they advance toward camera through the chamber's ambient light, each figure resolves from silhouette into clear, detailed form — faces, robes/forms, and expressions becoming visible for the first time.

Style: ultra-realistic character rendering merging with the mythic-tech environment, cinematic hero entrance staging (a confident ensemble walking toward camera in slow motion), each character's color palette rendered vividly and consistently (gold / blue-white / gold-green / deep red), no text.
Mood: confidence and gravity arriving into a scene of chaos — the adults have entered the room.
Camera: steady, slightly low-angle tracking shot moving backward in front of the four as they walk forward, classic slow-motion ensemble walk framing, no cuts.
Audio: the stuttering hum resolves into a stronger, more grounded rhythmic pulse as the four appear — a confident, arriving theme, brass or deep strings entering underneath the existing texture.
Duration: 8 seconds. Aspect ratio: 16:9.
```

**Shot 4.4 (1:30–1:35) — they arrive, survey the chaos** — status: drafted, not yet generated

```
The four arrive together at a railed platform overlooking the full chamber below and come to a stop, standing shoulder to shoulder, looking down at the overflowing, struggling robot workforce beneath them. Shot from slightly behind and beside the group, favoring silhouette and posture over close facial detail: the gold-glowing central figure stands thoughtful and still, hand resting on his ledger; the blue-white figure leans forward slightly, visibly alarmed, tablet held up and flashing warning symbols; the gold-green figure watches with concern, one hand half-raised as if wanting to help; the deep-red figure stands relaxed, arms crossed, entirely unbothered by the chaos below. Below them, the full scale of the overloaded chamber is visible — orbs spilling, drones glitching, robots working frantically.

Style: same ultra-realistic mythic-tech style, wide establishing composition contrasting the four still figures above against the frantic motion below, clear color-coded silhouettes, no text.
Mood: the calm of authority meeting the chaos of an overwhelmed system — a turning point.
Camera: slow, subtle push-in on the group from behind/beside, holding on the contrast between their stillness and the motion below, no cuts.
Audio: the arrival theme settles into a suspended, tense hold — quieter, waiting for what comes next.
Duration: 5 seconds. Aspect ratio: 16:9.
```

**YOUR NOTES — AI video prompts**

```
(type here — paste kept takes, note what worked/didn't, keep style keywords consistent across shots)
```

---

## Song / music scratchpad

| Moment | Idea | Language | Status |
|--------|------|----------|--------|
| Title song (opening video) | "Navras Se Hai Kalpana" — plays as the 4 head to heaven | Hindi | Title only, needs full lyrics |
| Shringaar Ras — Scene 2 (video) | Shringaar teaching song — "Shringar" line explained to Maya | Hindi | Teaching song TBD / lyrics to write |
| Shringaar Ras — Scene 2 (stage) | **"Ishq Wala Love"** + 2 more songs TBD | Hindi | 1 of 3 confirmed |
| Sponsorship celebration (Scene 8) | "Woh Sikandar Bhi Doston Ke Hain" — Team A/B + coach, video montage | Hindi | Confirmed song, choreo TBD |
| Karma hook | Catchy “karma” refrain | Mixed | |
| Hasya | Qawwali laughter riots | | |
| Raudra | Durandhar-*style* (original parody cue) | | |
| Finale | All-nine wow song | | |
| Bollywood snippets | Short, credited, comic timing | | |

**YOUR NOTES — music**

```
(type here)
```

---

## Props / costume scratchpad

**YOUR NOTES**

```
(type here)
```

---

# DESIGN — Livia, Karma, cinematic video, emotion build-up

## 1) Livia — character look (keep consistent)

**Base silhouette (pick and lock early):**
- Slightly humanoid / soft android — friendly, not scary.
- Condo-local: something that could “live” in Pasir Ris (balcony light, lift lobby, void deck vibe in B-roll).
- Face starts **incomplete**: soft blank face + a large empty **mega-emoticon outline** beside/behind Livia (no mini-emoticons yet).

**Consistent rules for every Livia shot:**
- Same face proportions, same “glow” colour for body (e.g. cool silver-blue).
- Same walk / head-tilt habit (comic: tilt when confused).
- Same UI overlay language (small text: `SCANNING…` `EMOTION?` `LOGGED`).
- Same **mega-emoticon shell** design every time — only the mini-emoticons inside change.
- Same frame treatment: mild digital grain or soft HUD corners — **always**, so audience knows “this is Livia-cam / Livia-world.”

**Voice / narration comic habits (optional):**
- Too literal (“Happiness equals teeth”).
- Wrong language mix when excited.
- Claps after tragedy, cries at jokes — until it learns.

---

## 2) Karma — character look (keep consistent)

**Base silhouette:**
- Mythic, warm, trickster — gold / saffron / deep indigo, not horror.
- Contrast with Livia: organic patterns (mandala, flame, lotus, peacock) vs Livia’s circuits.
- Always a **half-smile** or playful eyes — never pure wrath.

**Consistent rules:**
- Enters with the **same sound motif** (1–2 notes of hook song).
- Same visual “arrival”: swirl / rangoli bloom / smoke → form.
- Speaks to Livia like a mischievous elder / stage manager.
- When instant karma hits: **one shared gesture** (snap, eye-flash, bell) + hook sting.

---

## 3) Cinematic video approach (stage-friendly)

**Three layers on screen (simple, repeatable):**

| Layer | What | When |
|-------|------|------|
| A — World | Condo / nature / crowd B-roll | Always under |
| B — Characters | Livia and/or Karma (same design) | Scenes with them |
| C — Emotion HUD | Colour chips, face fragments, mudra cutaways | After each rasa unlock |

**Practical shooting tips:**
- Film **all Livia inserts** in one look-session (same light, same costume/VFX preset).
- Film **all Karma inserts** in one look-session.
- Shoot **condo B-roll** once: lifts, corridors, BBQ pit, car park, void deck — reuse under many scenes.
- For Durandhar-style anger: slow-mo walk, low angle, wind on shawl — then cut to **tiny** cause (cone / note on door). Comic edit = the joke.
- Keep clips short (5–15s) so live dance/song stays primary.

**Stage + screen sync:**
- Live ladies do mudras → screen flashes rasa name + colour.
- Unlock moment: freeze or soft light change on stage → mini-emoticon flies into Livia’s mega-emoticon on screen.
- Finale: mega-emoticon full with all 9 bouncing inside; stage is colourful chaos.

---

## 4) Creative emotion build-up (recommended system)

### Option chosen for this draft: **“Mega-Emoticon + 9 Mini-Emoticons Inside”**

**How it works:**
1. Beside / behind Livia: a large **empty emoticon outline** (soft grey circle face — the “shell”).
2. Each rasa unlocks a **mini-emoticon** (colour + expression) that **flies in and nests inside** the big shell.
3. Layout idea: 3×3 grid inside the big face, or orbit then settle into fixed slots.
4. Audience always sees **progress** (3/9, 6/9…) — comic and readable from the hall.
5. Finale: big emoticon **completes** — maybe its outer expression becomes a “wow / all emotions” face, while all 9 minis bounce inside.
6. Twist: outer face still looks complete, but **minis freeze, desaturate, or stop bouncing** — “you collected the faces; you don’t have karma.”

**Why this is strong:**
- Instantly readable (emoji language everyone knows).
- Comic by nature — fits whole-play humour.
- Easy motion graphics even if AI video drifts (CapCut / After Effects overlays).
- Finale image is iconic: one big face full of nine feelings.

**Staging tip:** Live mudras play; screen shows mini-emoticon “catch” into the mega face after each rasa.

---

## 5) Consistency checklist (print this)

- [ ] Livia face proportions never change — only emoticon inventory
- [ ] Karma arrival sound + look never change
- [ ] Each rasa mini-emoticon + colour locked before design
- [ ] Unlock animation is the **same** every time (mini flies into mega shell, 1–2 seconds)
- [ ] Mega-emoticon shell shape never changes
- [ ] HUD font / language style same in all videos
- [ ] Condo B-roll graded the same (one LUT / filter)
- [ ] Comedy first: every “epic” shot needs a human/petty punchline nearby

---

## Change log

| Date | What changed |
|------|----------------|
| (start) | First scribble + visual system |
| later | Switched emotion HUD from petals/mosaic to **mega-emoticon + 9 minis** |

**YOUR NOTES — open questions**

```
(type here)
```
